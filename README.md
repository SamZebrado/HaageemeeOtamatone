...
> 已修复之前的“按一次就没声”问题：现在手动按压在所有模式下都能正常持续出声。  
> 本项目只做启发和展示，暂时不保证在所有设备/浏览器上都完美可用。  
>  
> The old “goes silent after one press” bug is fixed: manual playing now sustains in all modes.  
> This project is for inspiration/demo only and not guaranteed to work perfectly on every device/browser.
